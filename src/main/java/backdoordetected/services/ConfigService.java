...
public class ConfigService {
  ...
  private ConfigService(File configFile) {
    this.configFile = configFile;
    loadConfig();
  }

  private ConfigService() {
    this(new File(DEFAULT_CONFIG_FILE_NAME));
  }
  ...
  static synchronized ConfigService getTestInstance(File configFile) {
    instance = new ConfigService(configFile);
    return instance;
  }
  ...
  private void loadConfig() {
    ...
    try (FileInputStream fis = new FileInputStream(configFile)) {
      ...
      logger.severe("Could not load " + configFile.getName() + ": " + e.getMessage());
    }
  }
  ...
  private void createDefaultConfig(File configFile) {
    try (FileOutputStream fos = new FileOutputStream(configFile)) {
      ...
      System.out.println("Created " + configFile.getName() + " file");
      System.out.println("Add your Gemini API keys to use AI features\n");
    } catch (IOException e) {
      logger.severe("Could not create " + configFile.getName() + ": " + e.getMessage());
    }
  }
