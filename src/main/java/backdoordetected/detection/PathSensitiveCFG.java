...
public class PathSensitiveCFG {
  ...
  public void analyzePaths(MethodNode methodNode, String className, Set<String> findings) {
    try {
      ...
      logger.info(
          String.format(
              "[PathSensitive] Analyzed %d paths in %s.%s",
  ...
  private Map<AbstractInsnNode, BasicBlock> buildBasicBlocks(
      MethodNode methodNode, CFGAnalyzer analyzer) {
  ...
  private BoolExpr computeBranchCondition(BasicBlock from, BasicBlock to) {
    ...
    Expr<?> ref =
        z3Context.mkConst(
            "ref" + System.identityHashCode(insn), z3Context.mkUninterpretedSort("Object"));
  ...
  private void reportVulnerability(
      CFGPath path, String className, String methodName, Set<String> findings) {
    ...
    String finding =
        String.format(
            ...
            sinkMethod, className.replace('/', '.'), methodName, path.getLength());
  ...
  private static class CFGPath {
    ...
    public CFGPath(
        BasicBlock currentBlock, BoolExpr pathCondition, List<BasicBlock> previousTrace) {
      this.currentBlock = currentBlock;
      this.pathCondition = pathCondition;
