...
public class PluginWorkerNew implements Runnable {
    ...
    public PluginWorkerNew(BlockingQueue<PrioritizedFile> queue,
    ...
    public void run() {
        ...
        try {
            while (true) {
                ...
                try {
                    if (scanMode == ScanMode.AI_MODERN || scanMode == ScanMode.AI
                            || scanMode == ScanMode.AI_BACKDOOR_FOCUS) {
                        orchestrator.scan(plugin.toPath(), scanMode, workerName);
                    } else {
                        logger.warning("[" + workerName
                                + "] Non-AI modes not yet migrated to new architecture. Using legacy implementation.");
                    }
                } catch (Exception e) {
                    logger.severe("[" + workerName + "] Error processing " + plugin.getName() + ": " + e.getMessage());
                    e.printStackTrace();
                }
            }
        } finally {
