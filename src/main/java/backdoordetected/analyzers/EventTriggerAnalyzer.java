...
public class EventTriggerAnalyzer implements PluginAnalyzer {

  private static final Logger logger = Logger.getLogger(EventTriggerAnalyzer.class.getName());
  private static final String ANALYZER_NAME = "EventTriggerAnalyzer";
  ...
  public PluginAnalysisResult analyze(
    ...
    for (Path file : javaFiles) {
      ...
      if (currentProgress > lastReportedProgress
          && (currentProgress % 10 == 0 || processedCount % 200 == 0)) {
        ...
        if (processedCount > 0 && elapsedTimeNs > 0) {
          ...
          System.out.printf(
              "\r[PROGRESS] Analyzing source files: %d%% (%d/%d). ETA: ~%d seconds...",
    ...
    boolean hasHighSeverity =
        allFindings.values().stream()
            ...
            .anyMatch(
                f ->
                    f.toUpperCase().startsWith("CRITICAL:") || f.toUpperCase().startsWith("HIGH:"));
    boolean hasLowSeverity =
        allFindings.values().stream()
            .flatMap(List::stream)
            .anyMatch(
                f -> f.toUpperCase().startsWith("LOW:") || f.toUpperCase().startsWith("MEDIUM:"));

    Map<String, List<String>> genericFindings = new HashMap<>();
    allFindings.forEach((path, list) -> genericFindings.put(path.getFileName().toString(), list));

    return new EventTriggerAnalysisResult(
        getName(), genericFindings, hasHighSeverity, hasLowSeverity, allFindings, failedFiles);
  }
  ...
  private static class EventTriggerVisitor extends VoidVisitorAdapter<Void> {
    ...
    public void visit(MethodDeclaration n, Void arg) {
      ...
      if (n.isAnnotationPresent("EventHandler")) {
        ...
        n.findAll(MethodCallExpr.class)
            .forEach(
                call -> {
            ...
            .ifPresent(
                scope -> {
                  if (scope.isMethodCallExpr()) {
                    ...
                    if ("getDecoder".equals(scopeCall.getNameAsString())
                          ...
                          if (encoded != null) {
                            ...
                            if (n.getParentNode().isPresent()
                              ...
                              ObjectCreationExpr parent =
                                  (ObjectCreationExpr) n.getParentNode().get();
    ...
    public void visit(ObjectCreationExpr n, Void arg) {
      ...
      if ("String".equals(n.getType().getNameAsString()) && n.getArguments().size() == 1) {
        n.getArgument(0)
            .ifArrayCreationExpr(
                arrayCreationExpr -> {
                  if (arrayCreationExpr.getElementType().isPrimitiveType()
                      && arrayCreationExpr.getElementType().asPrimitiveType().getType()
