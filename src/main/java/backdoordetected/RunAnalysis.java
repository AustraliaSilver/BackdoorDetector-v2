...
import backdoordetected.analyzers.DataFlowAnalyzer;
    ...
    try {
      DataFlowAnalysisResult result =
          (DataFlowAnalysisResult)
              analyzer.analyze(Collections.singletonList(testFile), workingDir);

      Map<String, List<String>> findings = result.getFileFindings();
      if (findings.isEmpty()) {
      ...
      if (!result.getFailedFiles().isEmpty()) {
        ...
        result.getFailedFiles().forEach(System.out::println);
      }
    } catch (Exception e) {
      e.printStackTrace();
    }
